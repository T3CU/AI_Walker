...
class Actor(nn.Module):
    ...
    def forward(self, state):
        "" "Creating an actor network for map states -> actions."""
        x = fu.relu((self.bn1(self.fc1(state))))
        x = fu.relu((self.bn2(self.fc2(x))))
        return fu.tanh(self.fc3(x))

class Critic(nn.Module):

    def __init__(self, state_size, action_size, seed, fcs1_units = 600, fcs2_units = 600, fca1_units = 300):
    ...
    def forward(self, state, action):
        ...
        xs = fu.relu((self.bn1(self.fcs1(state))))
        xs = self.fcs2(xs)
        xa = self.fca1(action)
        x = fu.relu(torch.add(xs, xa))
        return self.fc1(x)
